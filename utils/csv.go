...
func (cf *CloudflareIPData) toString() []string {
	...
	result[6]  = cf.Country
	result[7]  = cf.City
	result[8]  = strconv.FormatFloat(cf.Latitude, 'f', 4, 32)
	result[9]  = strconv.FormatFloat(cf.Longitude, 'f', 4, 32)
	result[10] = cf.CFColo
	return result
}
...
func ExportCsv(data []CloudflareIPData) {
	...
	_ = w.Write([]string{"IP 地址", "已发送", "已接收", "丢包率", "平均延迟", "下载速度 (MB/s)", "国家", "城市", "纬度", "经度", "CFColo"})
...
func (s DownloadSpeedSet) Print() {
	...
	for i := 0; i < PrintNum; i++ { // 如果要输出的 IP 中包含 IPv6，那么就需要调整一下间隔
		if len(dateString[i][0]) > 15 {
			headFormat = "%-40s%-5s%-5s%-5s%-6s%-14s%-16s%-14s%-8s%-8s%-8s\n"
			dataFormat = "%-42s%-8s%-8s%-8s%-10s%-18s%-18s%-16s%-10s%-10s%-10s\n"
			break
		}
	}
	fmt.Printf(headFormat, "IP 地址", "已发送", "已接收", "丢包率", "平均延迟", "下载速度 (MB/s)", "国家", "城市", "纬度", "经度", "CFColo")
	for i := 0; i < PrintNum; i++ {
		fmt.Printf(dataFormat, dateString[i][0], dateString[i][1], dateString[i][2], dateString[i][3], dateString[i][4], dateString[i][5], dateString[i][6], dateString[i][7], dateString[i][8], dateString[i][9], dateString[i][10])
	}
	if !noOutput() {
